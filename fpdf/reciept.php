...
if(isset($_COOKIE['reciept'])){
    $order_id = $_COOKIE['reciept'];
...
$query_run5 = mysqli_query($con,$query_name);
...
// Page header
    ...
    function Footer()
    {
        ...
        $this->SetFont('Arial','I',8);
                $this->Cell(0,10,'This is a computer generated invoice and dosent reqier the need for any physicl signature Page '.$this->PageNo().'/{nb}',0,0,'C');
    }
}
...
$pdf->SetFont('Times','',12);
    $pdf->Ln(20);
        $pdf->Cell(30,5,"Ordered by : ".$name,0,1,'L');
        ...
        $pdf->Cell(30,5,"Order Details ",0,1,'C');
        $pdf ->Ln();
        $pdf->Ln();
        $pdf->SetFont('Times','',12);
        $pdf->Cell(20);
        ...
        $pdf->Ln();
        $pdf->Cell(20);
        $pdf->Cell(140,5,'Net Total ',1,0,'L');
        $pdf->Cell(20,5,'$'.$net_total,1,0,'R');
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Ln();
        $pdf->Output();
